...
from PyQt6.QtWidgets import (
    ...
    QLabel, QMenuBar, QMenu, QMessageBox, QSpinBox, QPushButton, QCheckBox, QDialog
)
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self) -> None:
        ...
        phys_label = QLabel("Stanima(Physical):")
        ...
        mental_label = QLabel("Mana(Mental):")
    ...
    def _load_date_data(self, date_str: str) -> None:
        ...
        try:
            ...
            result = db.fetch_one("""
                SELECT physical, mental, hp, updated_at
                FROM daily_points
                WHERE date = ?
            """, (date_str,))
            ...
            if result:
                ...
                physical, mental, hp, updated_at = result
    ...
    def _save_daily_points(self) -> None:
        ...
        # Calculate HP
        raw_hp = physical + mental  # 0-20
        hp = 20 + (raw_hp * 4)   
        hp = max(20, min(100, hp))
        ...
        try:
            ...
            # UPSERT daily points
            db.execute("""
                INSERT INTO daily_points (date, physical, mental, hp, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?)
            ...
            """, (target_date, physical, mental, hp, now, now))
            
            print(f"Saved points for {target_date} at {now}: P={physical}, M={mental}, HP={hp}")
    ...
    def _show_settings_dialog(self) -> None:
        ...
        # ========== 新增：在 dialog 關閉後才應用設定 ==========
        if result == QDialog.DialogCode.Accepted:
            print("\n=== [MainWindow] Dialog closed, applying settings ===")
            self._apply_settings()
            print("=== [MainWindow] Settings applied ===\n")
        # ===================================================
    ...
    def _apply_settings(self) -> None:
        ...
        CRITICAL: Must be called AFTER settings dialog closes.
        """
        scale = self._settings.get_ui_scale()
        text_offset = self._settings.get_text_size_offset()
        
        print(f"\n=== Applying Settings: scale={scale:.2f}, offset={text_offset:+d} ===")
        ...
        # ========== CRITICAL: Apply font WITHOUT stylesheet interference ==========
        
        # Step 1: Set global application font
        app.setFont(base_font)
        print(f"[1/5] Set application font to {adjusted_size}pt")
        
        # Step 2: Force process events
        QCoreApplication.processEvents()
        
        # Step 3: Manually update ALL widgets (including hidden ones)
        all_widgets = app.allWidgets()
        print(f"[2/5] Updating {len(all_widgets)} widgets...")
        
        for widget in all_widgets:
            try:
                if widget is not None:
                    # Force font update (bypasses stylesheet)
                    widget.setFont(base_font)
            except RuntimeError:
                # Widget was deleted
                continue
        
        print("[3/5] All widgets updated")
        
        # Step 4: Process events again
        QCoreApplication.processEvents()
        
        # Step 5: Scale fixed-size widgets
        self._scale_widgets(scale)
        print("[4/5] Widget sizes scaled")
        
        # Step 6: Force complete UI rebuild
        ...
        QCoreApplication.processEvents()
        print("[5/5] UI rebuild complete")
        
        print(f"=== Settings Applied: Final font size = {adjusted_size}pt ===\n")
